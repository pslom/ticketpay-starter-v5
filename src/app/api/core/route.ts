...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (op === 'subscribe') {
      ...
      // Enqueue default alerts (posted, pre_late_fee_2d)
      try {
        if (id) {
          const { enqueueDefaultAlertsForSubscription } = await import('../../../lib/alerts');
          await enqueueDefaultAlertsForSubscription({
            subscriptionId: id,
            plate: plateNorm,
            state: stateNorm,
