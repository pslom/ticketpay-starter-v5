...
// Optional TLS relax gate (keeps current behavior if set to "1")
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 2) Find subscribed plates (SF) from DB
    ...
    try {
      ...
      client.release(); await pool.end().catch(()=>{});
    }

    if (!plates.length) {
      return j(200, { ok:true, db, note:'no_subscriptions', imported: 0 });
    }

    // 3) Fetch DataSF (48h window), map, filter to subscribed plates
    ...
    if (!filtered.length) {
      return j(200, { ok:true, db, imported: 0, fetched: ds.length, matched: 0 });
    }
    ...
    return j(200, {
      ...
      sync: { status: syncRes.status, body: syncBody }
    });
  } catch (e:any) {
