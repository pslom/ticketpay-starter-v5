...
export default function Home() {
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-10 space-y-8">
      ...
      <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-neutral-200 p-4" aria-describedby="form-help">
        ...
        <button
          type="submit"
          disabled={busy}
          className="h-12 w-full rounded-xl bg-black text-white font-medium hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-black/30 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {busy ? "Searching…" : "Search Tickets"}
        </button>

        {err && <p className="text-sm text-red-600" role="alert">{err}</p>}
        <p id="form-help" className="text-xs text-neutral-500">
          We only store your plate if you subscribe to alerts. Unsubscribe anytime.
        </p>
      </form>
