...
type Ticket = { citation_no:string; plate:string; state:string; amount_cents:number; status:"open"|"paid"|"void"; violation?:string; location?:string; issued_at?:string; due_at?:string; };
...
export default function ResultsPage(){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try{
        const res = await fetch("/api/lookup",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({plate,state})});
        if(!res.ok) throw new Error();
        const json = await res.json();
        if(!dead) setTickets(json.tickets||[]);
      }catch{ if(!dead) setErr("Can’t reach the city right now. Try again in a moment."); }
      finally{ if(!dead) setLoading(false); }
    })();
    return ()=>{dead=true};
  },[plate,state]);

  const total = useMemo(()=> tickets? tickets.filter(t=>t.status==="open").reduce((s,t)=>s+t.amount_cents,0):0,[tickets]);

  return (
    <main className="mx-auto max-w-md px-4 py-8 space-y-6">
      <header className="space-y-1">
        <h1 className="text-xl font-semibold">{`Tickets for ${plate} (${state})`}</h1>
        {total>0 && <p className="text-sm text-neutral-600">Total outstanding: <span className="font-medium">{dollars(total)}</span></p>}
      </header>
      ...
      {!loading && err && (
        <div className="rounded-2xl border border-[var(--border)] p-4">
          ...
          <button onClick={()=>location.reload()} className="mt-3 h-10 w-full rounded-lg border border-neutral-300">Retry</button>
        </div>
      )}
          ...
          {tickets.map(t=>(
            <div key={t.citation_no} className="rounded-2xl border border-[var(--border)] p-4">
              ...
              <div className="mt-3 flex items-center justify-between">
                <span className={`text-xs rounded px-2 py-1 ${t.status==="open"?"bg-red-50 text-red-700":"bg-green-50 text-green-700"}`}>{t.status.toUpperCase()}</span>
