...
const RATE_BUCKETS = {
  ...
  subscribe: { limit: 10, windowMs: 5 * 60_000 },
  preview: { limit: 3, windowMs: 60_000 },
} as const;
...
async function checkRate(name: keyof typeof RATE_BUCKETS, key: string) {
  ...
  if (now - rec.ts >= cfg.windowMs) { rec.count = 0; rec.ts = now; }
  rec.count++; bucket.set(key, rec);
...
export async function middleware(req: NextRequest) {
  ...
  // BYPASS: allow debug endpoints to pass unmodified
  if (url.pathname.startsWith("/api/debug/")) {
    const res = NextResponse.next();
    res.headers.set("X-Canonical-Host", CANONICAL_HOST);
    res.headers.set("X-Proto", (req.headers.get("x-forwarded-proto") || "").toLowerCase() || "n/a");
  ...
  if (ratelimit) {
    ...
    if (!r.success) return addSecurityHeaders(new NextResponse("Rate limit", { status: 429 }));
  } else {
    const ip = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || req.headers.get("x-real-ip") || "local";
    const now = Date.now();
    ...
    if (rec.count > 60) return addSecurityHeaders(new NextResponse("Rate limit", { status: 429 }));
  }

  // 4) Route-specific limits (prod only)
  if (!DEV) {
    const ip = req.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || req.headers.get("x-real-ip") || "local";
    if (url.pathname.startsWith("/api/lookup")) {
      const r = await checkRate("lookup", ip);
      if (!r.ok) return addSecurityHeaders(addRateHeaders(new NextResponse("Rate limit", { status: 429 }), r.limit, r.remaining, r.retryAfter));
