...
echo "🧩 Adding subscribe card..."
mkdir -p src/components
cat > src/components/AlertSubscribeCard.tsx <<'TSX'
'use client';
...
echo "🏗️ Updating layout to Manrope + gradient + OG metadata..."
mkdir -p src/app
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en" className={manrope.variable}>
      <body className="min-h-dvh bg-neutral-50 text-gray-900 antialiased
        [background:radial-gradient(900px_520px_at_50%_-160px,rgba(16,185,129,0.12),transparent_60%)]">
        {children}
      </body>
    </html>
  )
...
echo "🖥️ Results page shell with centered hero + card..."
  ...
  return (
    <main className="px-4">
      <header className="mx-auto max-w-5xl pt-10 sm:pt-16 text-center">
        ...
        <h1 className="mt-6 text-4xl/tight sm:text-5xl/tight font-extrabold">Alerts for your plate</h1>
        <p className="mt-3 text-lg text-gray-600">Get a heads-up when a new ticket posts. Unsubscribe anytime.</p>
      </header>

      <section className="mx-auto max-w-3xl pt-8 pb-16">
        <AlertSubscribeCard smsEnabled={false} manageHref="/manage" />
      </section>

      <footer className="mx-auto max-w-5xl pb-12 text-center text-sm text-gray-500">
        <p>✅ Secure · 🏛 Official data · 1-tap unsubscribe</p>
      </footer>
    </main>
  )
}
...
echo "📡 Minimal subscribe API stub..."
...
echo "✅ Apply-fractal-style complete. Now install icons if needed and run dev."
